...
module suitears::access_control {
    ...
    public fun add(admin: &Admin, self: &mut AccessControl, role: vector<u8>) {
        ...
        if (!self.contains(role)) { new_role_impl(self, role); };
    }
    ...
    public fun remove(admin: &Admin, self: &mut AccessControl, role: vector<u8>) {
        ...
        if (self.contains(role)) { self.roles.remove(&role); };
    }
    ...
    public fun grant(
        ...
        if (self.contains(role)) { (&mut self.roles[&role]).insert(new_admin); } else {
    ...
    public fun revoke(
        ...
        if (has_role_(old_admin, self, role)) { (&mut self.roles[&role]).remove(&old_admin); };
    }
