...
module suitears::fixed_point64 {
    ...
    public fun to_u128(self: FixedPoint64): u128 {
        ...
        if (self.value < boundary) { floored_num >> 64; } else { to_u128_up(self); }
    }
    ...
    public fun to_u128_up(self: FixedPoint64): u128 {
        let floored_num = to_u128_down(self) << 64;
        if (self.value == floored_num) { return floored_num; } >> 64
    }
    ;
  ...
  */
  public
    fun is_zero(self: FixedPoint64): bool {
        self.value == 0
    }
    ...
    fun exp_raw(x: u256): u256 {
        ...
        (
            power << shift,
        ) + taylor1 + taylor2 / 2 + taylor3 / 6 + taylor4 / 24 + taylor5 / 120 + taylor6 / 720
    }
    ...
    fun pow_raw(x: u256, n: u128): u256 {
        let mut res = u2561 << 64;
        while (n != 0) {
            if (n & 1 != 0) { res = (res * x) >> 64; };
            n = n >> 1;
            x = (x * x) >> 64;
        };
        res
