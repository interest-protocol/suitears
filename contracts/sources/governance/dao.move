...
module suitears::dao {
    ...
    public fun revoke_vote<DaoWitness: drop, CoinType>(
        ...
        let value = vote.balance.value();
        if (vote.agree) proposal.for_votes = proposal.for_votes - value
        else proposal.against_votes = proposal.against_votes - value;
    ...
    public fun unstake_vote<DaoWitness: drop, CoinType>(
        ...
        assert!(proposal_state_impl(proposal, c.timestamp_ms()) > ACTIVE, ECannotUnstakeFromAnActiveProposal);
        let proposal_id = object::id(proposal);
        assert!(proposal_id == vote.proposal_id, EVoteAndProposalIdMismatch);
    ...
    public fun execute<DaoWitness: drop, AuhorizedWitness: drop, Capability: key + store>(
        ...
        assert!(type_name::get<AuhorizedWitness>() == proposal.authorized_witness.extract(), EInvalidExecuteWitness);
    ...
    fun proposal_state_impl<DaoWitness: drop>(
        ...
        if (current_time < proposal.start_time) {
            ...
            // Active
            ACTIVE
        } else if (proposal.for_votes + proposal.against_votes == 0 ||
            proposal.for_votes <= proposal.against_votes ||
            proposal.for_votes + proposal.against_votes < proposal.quorum_votes ||
            proposal.voting_quorum_rate >
            div_down(proposal.for_votes, proposal.for_votes + proposal.against_votes)) {
            // Defeated
            DEFEATED
