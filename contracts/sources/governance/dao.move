...
module suitears::dao {
     ...
     * @return TypeName
     */
    public fun authorized_witness<DaoWitness: drop>(proposal: &Proposal<DaoWitness>): Option<
        TypeName
    > {
    ...
    public fun state<DaoWitness: drop>(proposal: &Proposal<DaoWitness>, c: &Clock): u8 {
        proposal_state_impl(proposal, c.timestamp_ms())
    }
    ...
    public fun cast_vote<DaoWitness: drop, CoinType>(
        ...
        assert!(proposal_state_impl(proposal, c.timestamp_ms()) == ACTIVE, EProposalMustBeActive);
        ...
        if (agree) { proposal.for_votes = proposal.for_votes + value; } else {
    ...
    public fun change_vote<DaoWitness: drop, CoinType>(
        ...
        emit(
            ChangeVote<DaoWitness, CoinType> {
                proposal_id,
                value,
                voter: ctx.sender(),
                end_time: proposal.end_time,
    ...
    public fun revoke_vote<DaoWitness: drop, CoinType>(
        ...
        emit(
            RevokeVote<DaoWitness, CoinType> {
                proposal_id: proposal_id,
                value,
                agree: vote.agree,
                voter: ctx.sender()
    ...
    public fun unstake_vote<DaoWitness: drop, CoinType>(
        ...
        assert!(
            proposal_state_impl(proposal, c.timestamp_ms()) > ACTIVE,
            ECannotUnstakeFromAnActiveProposal
        );
        let proposal_id = object::id(proposal);
        assert!(proposal_id == vote.proposal_id, EVoteAndProposalIdMismatch);

        emit(
            UnstakeVote<DaoWitness, CoinType> {
                proposal_id,
                value: vote.balance.value(),
                agree: vote.agree,
                voter: ctx.sender()
            }
        );
     ...
     */
    public fun queue<DaoWitness: drop>(proposal: &mut Proposal<DaoWitness>, c: &Clock) {
    ...
    public fun execute<DaoWitness: drop, AuhorizedWitness: drop, Capability: key + store>(
        ...
        assert!(
            type_name::get<AuhorizedWitness>() == proposal.authorized_witness.extract(),
            EInvalidExecuteWitness
        );

        let proposal_capability_id = proposal.capability_id.extract();
     ...
     */
    public fun return_capability<DaoWitness: drop, Capability: key + store>(
        dao: &Dao<DaoWitness>,
        cap: Capability,
        ...
        transfer::public_transfer(cap, dao.id.to_address());
    }
    ...
    public fun update_dao_config<DaoWitness: drop>(
        ...
        dao.voting_delay = voting_delay.destroy_with_default(dao.voting_delay);
        dao.voting_period = voting_period.destroy_with_default(dao.voting_period);
        dao.voting_quorum_rate = voting_quorum_rate.destroy_with_default(dao.voting_quorum_rate);
        dao.min_action_delay = min_action_delay.destroy_with_default(dao.min_action_delay);
        dao.min_quorum_votes = min_quorum_votes.destroy_with_default(dao.min_quorum_votes);

        assert!(
            1_000_000_000 >= dao.voting_quorum_rate && dao.voting_quorum_rate != 0,
            EInvalidQuorumRate
        );
    ...
    fun new_impl<OTW: drop, CoinType>(
        ...
        let treasury = dao_treasury::new<OTW>(dao_id.to_inner(), ctx);
        ...
        transfer::public_transfer(admin, dao.id.to_address());
     ...
     * @return Coin<CoinType>.
     */
    fun destroy_vote<DaoWitness: drop, CoinType>(
        vote: Vote<DaoWitness, CoinType>,
        ctx: &mut TxContext,
    ): Coin<CoinType> {
    ...
    fun proposal_state_impl<DaoWitness: drop>(
        ...
        if (current_time < proposal.start_time) {
            ...
            // Active;
            ACTIVE;
        } else if (proposal.for_votes + proposal.against_votes == 0 || proposal.for_votes <= proposal.against_votes || proposal.for_votes + proposal.against_votes < proposal.quorum_votes || proposal.voting_quorum_rate > div_down(
            proposal.for_votes,
            ...
            QUEUED;
        } else if (option::is_some(&proposal.capability_id)) { EXECUTABLE; } else { FINISHED; }
    }
