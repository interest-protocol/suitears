...
module suitears::farm_tests {
    ...
    fun test_stake() {
        ...
        next_tx(test, alice);
        {
            let mut farm = test::take_shared<Farm<S_ETH, SUI>>(test);
            let mut account = test::take_from_sender<Account<S_ETH, SUI>>(test);
        ...
        next_tx(test, alice);
        {
            ...
            assert_eq(
                farm::reward_debt(&account),
                (accrued_rewards_per_share * 1000) / (ETH_DECIMAL_SCALAR as u256)
            );
        ...
        next_tx(test, alice);
        {
            ...
            assert_eq(
                ...
                ((1000 * accrued_rewards_per_share) / (ETH_DECIMAL_SCALAR as u256)) - rewards_debt
            );
            assert_eq(
    ...
    fun test_unstake() {
        ...
        next_tx(test, alice);
        {
            ...
            let mut stake_coin = farm::unstake(&mut farm, &mut account, 300, &c, ctx(test));
            ...
            assert_eq(
                farm::reward_debt(&account),
                (accrued_rewards_per_share * 200) / (ETH_DECIMAL_SCALAR as u256)
            );
            assert_eq(farm::balance_stake_coin(&farm), 200);
            assert_eq(farm::last_reward_timestamp(&farm), 15_000 / 1000);
            assert_eq(
                accrued_rewards_per_share,
                (ETH_DECIMAL_SCALAR as u256) * ((REWARDS_PER_SECOND as u256) * 5) / 500
            );
    ...
    fun test_end_to_end() {
        ...
        let accrued_rewards_per_share = (REWARDS_PER_SECOND as u256) * 4 * (
            ETH_DECIMAL_SCALAR as u256,
        ) / 800 + (REWARDS_PER_SECOND as u256) * 5 * (ETH_DECIMAL_SCALAR as u256) / 500;
        ...
        next_tx(test, alice);
        {
            ...
            assert_eq(farm::last_reward_timestamp(&farm), 15_000 / 1000);
            assert_eq(
                farm::accrued_rewards_per_share(&farm),
                (REWARDS_PER_SECOND as u256) * 5 * (ETH_DECIMAL_SCALAR as u256) / 500
            );
            ...
            assert_eq(
                (burn_for_testing(reward_coin) as u256),
                accrued_rewards_per_share * 500 / (ETH_DECIMAL_SCALAR as u256)
            );
            assert_eq(farm::accrued_rewards_per_share(&farm), accrued_rewards_per_share);
            assert_eq(farm::amount(&account), 200);
            assert_eq(
                farm::reward_debt(&account),
                200 * accrued_rewards_per_share / (ETH_DECIMAL_SCALAR as u256)
            );
            assert_eq(farm::balance_stake_coin(&farm), 500);
        ...
        next_tx(test, bob);
        {
            ...
            assert_eq(
                farm::reward_debt(&account),
                (300 * (REWARDS_PER_SECOND as u256) * 5 * (ETH_DECIMAL_SCALAR as u256) / 500) / (
                    ETH_DECIMAL_SCALAR as u256,
                )
            );
            ...
            assert_eq(
                (burn_for_testing(reward_coin) as u256),
                (300 * (REWARDS_PER_SECOND as u256) * 4 * (ETH_DECIMAL_SCALAR as u256) / 800) / (
                    ETH_DECIMAL_SCALAR as u256,
                )
            );
            ...
            assert_eq(
                farm::reward_debt(&account),
                400 * accrued_rewards_per_share / (ETH_DECIMAL_SCALAR as u256)
            );
    ...
    fun test_no_rewards() {
        ...
        next_tx(test, alice);
        {
            ...
            let mut cap = farm::new_cap(ctx(test));
            let mut farm = farm::new_farm<S_ETH, SUI>(
                &mut cap,
                &eth_metadata,
                &c,
                REWARDS_PER_SECOND,
        ...
        next_tx(test, bob);
        {
            ...
            let mut stake_coin = farm::unstake(&mut farm, &mut account, 100, &c, ctx(test));
            ...
            assert_eq(
                farm::reward_debt(&account),
                (accrued_rewards_per_share * 150) / (ETH_DECIMAL_SCALAR as u256)
            );
    ...
    fun test_update_rewards_per_second() {
        ...
        next_tx(test, alice);
        {
            ...
            farm::update_rewards_per_second(&mut farm, &cap, 1, &c);
    ...
    fun test_wrong_admin_update_rewards() {
        ...
        next_tx(test, alice);
        {
            ...
            farm::update_rewards_per_second(&mut farm, &wrong_cap, 1, &c);
    ...
    fun test_invalid_unstake_amount() {
        ...
        next_tx(test, alice);
        {
            ...
            let mut stake_coin = farm::unstake(&mut farm, &mut account, 501, &c, ctx(test));
    ...
    fun test_stake_invalid_account() {
        ...
        next_tx(test, alice);
        {
            ...
            let mut wrong_account = farm::new_account(&farm2, ctx(test));
    ...
    fun test_unstake_invalid_account() {
        ...
        next_tx(test, alice);
        {
            ...
            let mut stake_coin = farm::unstake(&mut farm, &mut wrong_account, 100, &c, ctx(test));
