...
module suitears::linear_vesting_wallet_clawback_tests {
    ...
    fun test_end_to_end_with_no_clawback() {
        ...
        let (owner_cap, recipient_cap, mut wallet) = linear_vesting_wallet_clawback::new(
            total_coin,
            &c,
            start,
            end,
        ...
        assert_eq(
            coin::burn_for_testing(
                linear_vesting_wallet_clawback::claim(&mut wallet, &recipient_cap, &c, &mut ctx),
            ),
            first_claim,
        );
    ...
    fun test_end_to_end_with_clawback() {
        ...
        assert_eq(
            coin::burn_for_testing(
                linear_vesting_wallet_clawback::clawback(&mut wallet, owner_cap, &c, &mut ctx),
            ),
            third_claim,
        );
        ...
        assert_eq(linear_vesting_wallet_clawback::balance(&wallet), 0);
    ...
    fun test_invalid_start_time() {
        ...
        let (owner_cap, recipient_cap, wallet) = linear_vesting_wallet_clawback::new(
            total_coin,
            &c,
            start,
            end,
