...
module suitears::math64_tests {
    ...
    #[test]
    fun test_try_add() {
        let mut pred = try_add(5678, 1234);
        assert_eq(pred, true);
        assert_eq(result, 5678 + 1234);

        let mut pred = try_add(MAX_U64, 1);
    ...
    #[test]
    fun test_try_sub() {
        let mut pred = try_sub(5678, 1234);
        assert_eq(pred, true);
        assert_eq(result, 5678 - 1234);

        let mut pred = try_sub(1234, 5678);
        assert_eq(pred, false);
        assert_eq(result, 0);
    }
    ...
    #[test]
    fun test_try_mul() {
        let mut pred = try_mul(5678, 1234);
        assert_eq(pred, true);
        assert_eq(result, 5678 * 1234);

        let mut pred = try_mul(5678, 0);
        assert_eq(pred, true);
        assert_eq(result, 0);

        let mut pred = try_mul((MAX_U64 / 2) + 1, 2);
        assert_eq(pred, false);
        assert_eq(result, 0);
    }
    ...
    fun test_try_div_down() {
        let mut pred = try_div_down(0, 1234);
        assert_eq(pred, true);
        ...
        let mut pred = try_div_down(7000, 5678);
        assert_eq(pred, true);
        assert_eq(result, 7000 / 5678);

        let mut pred = try_div_down(7000, 0);
        assert_eq(pred, false);
        assert_eq(result, 0);
    }
    ...
    #[test]
    fun test_try_div_up() {
        let mut pred = try_div_up(0, 1234);
        assert_eq(pred, true);
        assert_eq(result, 0 / 1234);
        ...
        let mut pred = try_div_up(7000, 5678);
        assert_eq(pred, true);
        assert_eq(result, 2);

        let mut pred = try_div_up(7000, 0);
        assert_eq(pred, false);
        assert_eq(result, 0);
    }
    ...
    fun test_try_mul_div_down() {
        let mut pred = try_mul_div_down(10, 2, 4);
        assert_eq(pred, true);
        assert_eq(result, 5);

        let mut pred = try_mul_div_down((MAX_U64 / 2) + 1, 2, 4);
        assert_eq(pred, true);
        assert_eq(result, 4611686018427387904);

        let mut pred = try_mul_div_down(10, 2, 0);
        assert_eq(pred, false);
        assert_eq(result, 0);

        let mut pred = try_mul_div_down(1, 7, 2);
        assert_eq(pred, true);
        assert_eq(result, 3);

        let mut pred = try_mul_div_down(MAX_U64, 2, 2);
        assert_eq(pred, true);
    ...
    fun test_try_mul_div_up() {
        let mut pred = try_mul_div_up(10, 2, 4);
        assert_eq(pred, true);
        assert_eq(result, 5);

        let mut pred = try_mul_div_up((MAX_U64 / 2) + 1, 2, 4);
        assert_eq(pred, true);
        assert_eq(result, 4611686018427387904);

        let mut pred = try_mul_div_up(10, 2, 0);
        assert_eq(pred, false);
        assert_eq(result, 0);

        let mut pred = try_mul_div_up(1, 7, 2);
        assert_eq(pred, true);
        assert_eq(result, 4);
    }
    ...
    #[test]
    fun test_try_mod() {
        let mut pred = try_mod(284, 5678);
        assert_eq(pred, true);
        assert_eq(result, 284 % 5678);

        let mut pred = try_mod(17034, 5678);
        assert_eq(pred, true);
        assert_eq(result, 17034 % 5678);

        let mut pred = try_mod(5678, 0);
        assert_eq(pred, false);
        assert_eq(result, 0);
    }
