...
module suitears::fixed_point64_tests {
    ...
    fun test_mul() {
        ...
        let result = fixed_point64::mul(
            fixed_point64::from_raw_value(340282366920938463463374607431768211455 / 2),
            fixed_point64::from(2),
        );
    ...
    #[test]
    fun test_mul_div() {
        let result = fixed_point64::mul_div(
            fixed_point64::from(5),
            fixed_point64::from(2),
            fixed_point64::from(3),
        );

        assert_eq(
            fixed_point64::value(result),
            ((((5 as u256) << 64) * (2 << 64) / (3 << 64)) as u128),
        );
    }
    ...
    fun test_mul_u128() {
        ...
        let y = fixed_point64::from_rational(1, 3);
        // 0.33
        let result = fixed_point64::mul_u128(9, y);

        assert_eq(result, 2);

        let result = fixed_point64::mul_u128(
            9,
            fixed_point64::from_raw_value(fixed_point64::value(y) + 1),
        );
    ...
    fun test_conversion_functions() {
        let x = fixed_point64::from_rational(499, 1000);
        // 0.499
        let result = fixed_point64::to_u128(x);
        assert_eq(result, 0);

        let x = fixed_point64::from_rational(1, 2);
        // 0.5
        let result = fixed_point64::to_u128(x);
        assert_eq(result, 1);

        let x = fixed_point64::from_rational(1, 1);
        // 1
        ...
        let x = fixed_point64::from_rational(7, 2);
        // 3.5
        let result = fixed_point64::to_u128_down(x);
        assert_eq(result, 3);
    }
