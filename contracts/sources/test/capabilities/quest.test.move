...
module suitears::quest_tests {
    ...
    public struct Reward has store, drop { value: u64 }

    #[test]
    fun test_success_case() {
        let mut ctx = tx_context::dummy();

        let mut required_tasks = vec_set::empty();
        ...
        let mut q = quest::new(required_tasks, Reward {value: 7}, &mut ctx);

        assert_eq(
            quest::required_tasks(&q),
            vector[type_name::get<task_one::Witness>(), type_name::get<task_two::Witness>()],
        );
