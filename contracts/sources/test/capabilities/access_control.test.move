...
module suitears::acess_control_tests {
    ...
    #[test]
    fun test_new() {
        let mut ac = access_control::new(&mut dummy());

        assert_eq(access_control::access_control(&super_admin), id_address(&ac));
        assert_eq(
            access_control::has_role(&super_admin, &ac, access_control::super_admin_role()),
            true
        );
    ...
    fun test_renounce_wrong_access_control() {
        let mut ac = access_control::new(&mut dummy());
        let mut ac2 = access_control::new(&mut new_from_hint(@0x5, 1, 2, 3, 4));
