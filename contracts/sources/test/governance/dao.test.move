...
module suitears::dao_tests {
    ...
    public struct AuthorizedWitness has drop {}
    ...
    fun initiates_correctly() {
        ...
        next_tx(test, alice);
        {
            ...
            assert_eq(
                *option::borrow(&dao::authorized_witness(&proposal)),
    ...
    fun test_vote_mechanisms() {
        ...
        {
            let mut proposal = test::take_shared<Proposal<InterestDAO>>(test);
            let mut vote = test::take_from_sender<Vote<InterestDAO, S_ETH>>(test);

            assert_eq(dao::for_votes(&proposal), 0);
            assert_eq(dao::against_votes(&proposal), 900);

            dao::change_vote<InterestDAO, S_ETH>(&mut proposal, &mut vote, &c, ctx(test));
    ...
    fun test_no_otw_dao() {
        ...
        next_tx(test, alice);
        {
            let mut dao = dao::new<InterestDAO, S_ETH>(
                InterestDAO {  },
                DAO_VOTING_DELAY,
                DAO_VOTING_PERIOD,
                DAO_QUORUM_RATE,
    ...
    fun test_zero_dao_quorum_rate() {
        ...
        next_tx(test, alice);
        {
            let mut dao = dao::new_for_testing<InterestDAO, S_ETH>(
                DAO_VOTING_DELAY,
                DAO_VOTING_PERIOD,
                0,
                DAO_MIN_ACTION_DELAY,
    ...
    fun test_change_vote_on_agreed_proposal() {
        ...
        next_tx(test, alice);
        {
            ...
            dao::change_vote(&mut proposal, &mut vote, &c, ctx(test));
    ...
    fun test_change_vote_on_wrong_proposal() {
        ...
        next_tx(test, alice);
        {
            let mut proposal = test::take_shared_by_id<Proposal<InterestDAO>>(
                test,
                *option::borrow(&proposal1_id)
            );
        ...
        next_tx(test, alice);
        {
            let proposal = test::take_shared_by_id<Proposal<InterestDAO>>(
                test,
                *option::borrow(&proposal1_id)
            );
            let dao = test::take_shared<Dao<InterestDAO>>(test);
            let mut proposal2 = test::take_shared_by_id<Proposal<InterestDAO>>(
                test,
                *option::borrow(&proposal2_id)
            );

            let mut vote = test::take_from_sender<Vote<InterestDAO, S_ETH>>(test);

            dao::change_vote(&mut proposal2, &mut vote, &c, ctx(test));
    ...
    fun test_revoke_vote_on_pending_proposal() {
        ...
        next_tx(test, alice);
        {
            ...
            burn_for_testing(dao::revoke_vote(&mut proposal, vote, &c, ctx(test)));
    ...
    fun test_revoke_vote_on_wrong_proposal() {
        ...
        next_tx(test, alice);
        {
            ...
            let dao = test::take_shared<Dao<InterestDAO>>(test);
            let proposal2 = test::take_shared_by_id<Proposal<InterestDAO>>(
                test,
                *option::borrow(&proposal2_id)
            );
    ...
    fun test_unstake_vote_on_pending_proposal() {
        ...
        next_tx(test, alice);
        {
            ...
            burn_for_testing(dao::unstake_vote(&proposal, vote, &c, ctx(test)));
