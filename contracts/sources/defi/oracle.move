...
module suitears::oracle {
    ...
    public fun request<Witness: drop>(self: &Oracle<Witness>): Request {
        ...
        Request {oracle: object::id(self), feeds: vec_set::empty(), reports: vector[]}
    }
    ...
    public fun report<Witness: drop>(
        ...
        vector::push_back(
            &mut request.reports,
            Report {
                price: math256::mul_div_down(
                    (price as u256),
                    WAD,
                    math256::pow(10, (decimals as u256)),
                ),
                timestamp,
            },
        );
     ...
     */
    public fun destroy_request<Witness: drop>(
        self: &Oracle<Witness>,
        request: Request,
        c: &Clock,
    ): Price {
        ...
        Price {oracle, price: leader_price, decimals: 18, timestamp: leader_timestamp}
    }
     ...
     * - the `cap` is not the owner of `self`.
     */
    public fun uid_mut<Witness: drop>(
        self: &mut Oracle<Witness>,
        cap: &OwnerCap<Witness>,
    ): &mut UID {
        owner::assert_ownership(cap, object::id(self));
        &mut self.id
    }
     ...
     */
    public fun remove<Witness: drop>(
        self: &mut Oracle<Witness>,
        cap: &OwnerCap<Witness>,
        feed: TypeName,
    ) {
     ...
     */
    public fun update_time_limit<Witness: drop>(
        self: &mut Oracle<Witness>,
        cap: &OwnerCap<Witness>,
        time_limit: u64,
    ) {
     ...
     */
    public fun update_deviation<Witness: drop>(
        self: &mut Oracle<Witness>,
        cap: &OwnerCap<Witness>,
        deviation: u256,
    ) {
     ...
     */
    public fun new_price_for_testing(oracle: ID, price: u256, decimals: u8, timestamp: u64): Price {
        Price {oracle, price, decimals, timestamp}
    }
}
