...
module suitears::linear_vesting_wallet_clawback {
     ...
     */
    public fun new<T>(token: Coin<T>, c: &Clock, start: u64, duration: u64, ctx: &mut TxContext): (
        OwnerCap<ClawBackWitness>,
        OwnerCap<RecipientWitness>,
        Wallet<T>
    ) {
    ...
    public fun claim<T>(
        ...
        coin::from_balance(
            balance::split(&mut self.balance, math64::min(releasable, current_balance)),
            ctx
        )
    }
