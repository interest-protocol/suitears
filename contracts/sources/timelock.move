...
module suimate::timelock {
  ...
  public fun create<T: drop>(_: T, unlock_epoch: u64, allow_extensions: bool, can_update_unlock_epoch: bool, ctx: &mut TxContext): TimeLock<T> {
  ...
  public fun destroy<T: drop>(timelock: TimeLock<T>) {
    let TimeLock { id, allow_extensions: _, unlock_epoch: _, can_update_unlock_epoch: _ } = timelock;
    object::delete(id);
  } 
  ...
  public fun update_unlock_epoch<T>(timelock: &mut TimeLock<T>, unlock_epoch: u64) {
