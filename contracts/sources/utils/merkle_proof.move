...
module suitears::merkle_proof {
     ...
     */
    public fun verify_with_index(proof: &vector<vector<u8>>, root: vector<u8>, leaf: vector<u8>): (
        bool,
        u256
    ) {
        ...
        while (i < proof_length) {
            ...
            computed_hash = if (vectors::lt(computed_hash, proof_element)) efficient_hash(
                computed_hash,
                proof_element
            ) else {
                j = j + 1;
